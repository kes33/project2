...
RC SqlEngine::select(int attr, const string &table, const vector<SelCond>&conds) {
  ...
  // open the table file
	...
	if (!indexExists) {
		...
		//sort the conditions by key or value
		vector<SelCond> keyConds, valueConds;
		int i;
		int numConds = conds.size();
		cout << "there are "<< numConds << " conditions specified by user" << endl;
		for (i = 0; i < numConds; i++) {
			if (conds[i].attr == 1)
				keyConds.push_back(conds[i]);
			else valueConds.push_back(conds[i]);
		}

		//check for the existence of conditions necessitating the index
		if (conds.empty() || keyConds.empty() || keyConds[0].comp==SelCond::NE) {
		...
		//otherwise, sort the vector of key conditions
